...
void ProcessAnalogData(AnalogBufferDMA *pabdma, int8_t adc_num1) {
  // if(adc_num1 == 0){
  //  triggerIndex = get_trigger_index(pabdma);
  // }
    //long hscale;
    //long vscale
 ...
 // volatile uint16_t *end_pbuffer2 = pbuffer2 + pabdma->bufferCountLastISRFilled();
  ...
  int16_t x = 0;
    ...
    tft.drawFastHLine(0, trigger*10/vScale+100, 280, ILI9341_YELLOW);
  while (pbuffer < end_pbuffer && (x*100/hScale+40) < tft.width()) {
    //Serial.println(*pbuffer);
    int y1 = tft.height() / 2 - (*pbuffer *6.66* 10 / vScale);
   // int y2 = tft.height() / 2 - (*pbuffer2 *6.66* 10 / vScale);
    if(adc_num1 == 0){
       tft.drawPixel(x*100/hScale,pbuffer[triggerIndex-140+x]*10/vScale+100,ILI9341_GREEN);
      // Serial.print("\nCh1 voltage:");
     //  Serial.print(pbuffer[triggerIndex-140+x]*10/vScale+100);
      //     Serial.print("  Trigger level: ");
    //Serial.print(trigger);
   // tft.drawPixel(x*(200/hScale), y1, ILI9341_GREEN);
    //tft.drawPixel(x, y2, ILI9341_BLUE);
    }else{
       //tft.drawPixel(x*(200/hScale), y1, ILI9341_BLUE);
       tft.drawPixel(x*100/hScale,pbuffer[triggerIndex-140+x]*10/vScale+100,ILI9341_BLUE);
              Serial.print("\nCh2 voltage:");
       Serial.print(pbuffer[triggerIndex-140+x]*10/vScale+100);
           Serial.print("  Trigger level: ");
    Serial.print(trigger);
...
// int get_trigger_index(AnalogBufferDMA *dma_buffer_in){
//   int triggerIndex;
...
//   for(volatile uint16_t  *i = 140; i < end_pbuffer;i++){
//      Serial.print("    i:");
//   Serial.print(i);
